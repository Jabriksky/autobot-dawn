...
async function worker(index, token, proxyLines, proxyMode) {
  ...
  while (!stopped) {
    ...
    if (proxyToUse) console.log(`[INFO][${id}] Using proxy: ${proxyToUse}`);
    else console.log(`[INFO][${id}] No proxy`);

    try {
      const now = new Date().toISOString().replace('T',' ').slice(0,19);
      const res = await axiosInst.get(URL);
      const bodySummary = (typeof res.data === 'string')
        ? res.data.slice(0,200)
        : JSON.stringify(res.data).slice(0,200);
      console.log(`[${now}][${id}] ${res.status} | ${bodySummary}`);
    } catch (err) {
      console.error(`[ERR][${id}] ${err.message || String(err)}`);
    }
